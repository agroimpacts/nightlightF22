...
In order to account for how different characteristics of the urban environment might impact the nighttime brightness, we grouped the nightlight data based on building structure and population to compare brightness by like areas. In order to do this, the four gridded datasets are stacked:
...
ggsave(regressplt,file = here("vignettes/figures/regression.png"), width = 7)
```
...
bxplt <- ggplot(combined) + 
  ...
  theme_linedraw()
ggsave(bxplt, file = here("vignettes/figures/ltmean_boxplot.png"), 
       height = 7, width = 10, dpi = 72, bg = "grey")
```

```{r, echo=FALSE}
knitr::include_graphics(here("vignettes/figures/ltmean_boxplot.png"))
```
